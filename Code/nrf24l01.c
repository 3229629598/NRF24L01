...
const uint8_t M_ADDRESS[TX_ADR_WIDTH]={0x78,0x78,0x78,0x78,0x78};//主机PTX和P0,从机P1地址
const uint8_t S_ADDRESS[TX_ADR_WIDTH]={0x50,0x50,0x50,0x50,0x50};//从机PTX和P0,主机P1地址
...
uint8_t NRF24L01_TxPacket(uint8_t *txbuf)
{
	uint8_t sta;
	NRF24L01_CE_LOW();
  NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  28个字节
...
{
	NRF24L01_CE_LOW();
	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P1,(uint8_t*)S_ADDRESS,RX_ADR_WIDTH);//写P1地址
...
{
	NRF24L01_CE_LOW();
	NRF24L01_Write_Buf(NRF_WRITE_REG+TX_ADDR,(uint8_t*)M_ADDRESS,TX_ADR_WIDTH);//写TX地址
	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(uint8_t*)M_ADDRESS,RX_ADR_WIDTH);//写P0地址
	NRF24L01_Write_Reg(NRF_WRITE_REG+EN_AA,0x01);    //使能通道0的自动应答    
  NRF24L01_Write_Reg(NRF_WRITE_REG+EN_RXADDR,0x01);//使能通道0的接收地址
	NRF24L01_Write_Reg(NRF_WRITE_REG+SETUP_RETR,0x1a);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次	
  NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,30);	     //设置RF通信频率		  
...
{
	NRF24L01_CE_LOW();
	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P1,(uint8_t*)M_ADDRESS,RX_ADR_WIDTH);//写P1地址
	NRF24L01_Write_Reg(NRF_WRITE_REG+EN_AA,0x02);    //使能通道1的自动应答    
  NRF24L01_Write_Reg(NRF_WRITE_REG+EN_RXADDR,0x02);//使能通道1的接收地址
  NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,30);	     //设置RF通信频率		  
	NRF24L01_Write_Reg(NRF_WRITE_REG+RX_PW_P1,RX_PLOAD_WIDTH);//选择通道1的有效数据宽度
...
void NRF24L01_STX_Mode(void)
{
	NRF24L01_CE_LOW();
	NRF24L01_Write_Buf(NRF_WRITE_REG+TX_ADDR,(uint8_t*)S_ADDRESS,TX_ADR_WIDTH);//写TX地址
	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(uint8_t*)S_ADDRESS,RX_ADR_WIDTH);//写P0地址
	NRF24L01_Write_Reg(NRF_WRITE_REG+EN_AA,0x01);    //使能通道0的自动应答    
  NRF24L01_Write_Reg(NRF_WRITE_REG+EN_RXADDR,0x01);//使能通道0的接收地址
	NRF24L01_Write_Reg(NRF_WRITE_REG+SETUP_RETR,0x1a);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次	
  NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,30);	     //设置RF通信频率		  
